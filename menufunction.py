...
class ProductIcon:
    ...
    def change_status(self, event=None):
        ...
        product_id = self.table_product.item(select)['values'][0]
        ...
        RadioButton1 = ttk.Radiobutton(
            StatusGUI, text='Show icon', variable=self.v_radio, value='show', command=lambda x=None: InsertProductStatus(int(product_id), 'show'))
        RadioButton2 = ttk.Radiobutton(
            StatusGUI, text='Not show icon', variable=self.v_radio, value='', command=lambda x=None: InsertProductStatus(int(product_id), ''))
        ...
        print('check :', check)
        if check[-1] == 'show':
            RadioButton1.invoke()  # ຖ້າສະຖານະ show ຈະເລືອກ show icon
        else:
            RadioButton2.invoke()
        ...
        def check_close():
            print('closed')
            StatusGUI.destroy()  # close window
            self.insert_table()

        StatusGUI.protocol('WM_DELETE_WINDOW', check_close)
